...
/** Self-service account panel (profile edit + password change + logout).
 *  Shared by the store admin and the super-admin (platform) panels. */
export function AccountPanel() {
  ...
  async function changePassword() {
    ...
    try {
      ...
      setPwMsg({ ok: false, text: e instanceof ApiError ? e.message : 'Could not change password' });
    } finally {
      setSavingPw(false);
    }
  }
  ...
  return (
    <div className="max-w-xl space-y-6">
      <PageHeader title="My account" action={<Button variant="outline" onClick={logout}>Logout</Button>} />
      ...
      <Card className="space-y-4">
        ...
        <Input label="New password (min 8 characters)" type="password" autoComplete="new-password" value={next} onChange={(e) => setNext(e.target.value)} />
        ...
        <div className="flex items-center gap-3">
          <Button onClick={changePassword} disabled={savingPw || !current || !next}>{savingPw ? 'Updating…' : 'Update password'}</Button>
